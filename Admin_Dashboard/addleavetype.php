            ...
            <div class="card">
			  ...
			  <a href="#"><button type="button" class="btn btn-primary" onclick="window.print();">
                ...
                <h3 class="card-title" style="float: right;"><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-overlay">
                  <?php echo htmlspecialchars($lang['alt']);?>
                </button></h3>
                <?php include 'modal/addleavetypemodal.php';?>
              </div>
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
					  ...
					  $dbs = new database();
					  $db=$dbs->connection();
                      $result = mysqli_query($db,"SELECT l.*,e.employee_firstname,e.employee_lastname,lt.Type_of_Name FROM leavedetails l JOIN employee_table e ON l.EmpId=e.employee_id JOIN type_of_leave lt on l.TypesLeaveId=lt.LeaveId ");
                      // if($result->num_rows === 0) echo'<div class="alert alert-warning" role="alert">No Record Found!!!</div>';
                       while ($row = $result->fetch_array()){
                        $name = ucfirst($row['employee_firstname'])."&nbsp;".ucfirst($row['employee_lastname']);
                      ?>
                      <tr>

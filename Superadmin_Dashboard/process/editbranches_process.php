...
<?php
  if(isset($_POST['update2'])){
    ...
    $startd = date('Y-m-d', strtotime($date));
       ...
       $sql = $phdb->query("UPDATE `branches_table`  SET `branches_name` = '$branches_name' ,`date_created` ='$startd' WHERE branches_id = '$branches_id'")  or die (mysqli_error($phdb));
       if($sql == TRUE){
        	           ?>
